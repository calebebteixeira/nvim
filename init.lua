...
-- Save
vim.keymap.set("n", "<leader>w", ":w!<cr>", { desc = "Move selected text down" })
-- Quit
...
require("lazy").setup({
	...
	},
	{
		"ggandor/leap.nvim",
		config = function()
			vim.keymap.set({ "n", "v" }, "s", "<Plug>(leap-forward)")
			vim.keymap.set({ "n", "v" }, "S", "<Plug>(leap-backward)")
		end,
	},
	-- {
	...
	},
	{
		...
		config = function()
			vim.keymap.set(
				...
				{ desc = "[S]earch and replace with Perl like Regex" }
			)
		end,
	},
	{
